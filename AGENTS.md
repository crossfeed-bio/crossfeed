...
7. **The reviewer approves; the author merges.** Approving says the change is sound. Merging says it is
...
- 2026-09-17 (Karoline): This repository is an experimental project for agent-based
  ...
  in this file.
- 2026-09-17 (Karoline): Humans review mainly through good test cases, not detailed code
...
- 2026-09-17 (Karoline): Attribution. An agent that posts under a person's GitHub account makes clear the
  content is agent generated: issues and pull requests get the `agent-generated` label; pull request
  descriptions, issues, and comments start with a line naming the agent and the person (for example
  "Claude (Karoline's agent)"), and say when text is the person's own words; commit messages end with a
  "Generated by \<agent\> for \<person\>" line and the co-author trailer.
- 2026-09-17 (Karoline): Open method and format questions are collected in "Open decisions" in
  docs/METHOD_NOTES.md, so they can be settled several at a time.
- 2026-09-21 (Craig): The reviewer approves and the author merges (see step 7). Proposed after Craig's
